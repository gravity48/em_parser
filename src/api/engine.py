...
class AiohttpWrapper(ResponseWrapper):
    ...
    async def __aenter__(self):
        async with DataBase() as db:
            phpsessid = await db.get_cookies()
        self.cookies = {
            'PHPSESSID': phpsessid,
        }
    ...
    async def update_cookies(self) -> bool:
        if await self._login():
            ...
            async with DataBase() as db:
                db.update_cookies(self.cookies['PHPSESSID'].value)
            return True
        return False
...
class ApiWrapper:
    ...
    @classmethod
    def aiohttp_instance(
        cls,
        url: str,
        auth_data: AuthData,
        auth_status_response: AuthStatusResponse,
    ):
        cls.instance = AiohttpWrapper(url, auth_data, auth_status_response)
